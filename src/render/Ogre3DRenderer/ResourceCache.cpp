...
void Ogre3DResourceCache::InitRoot()
{
   ...
   {
      const auto section_name = section_iter.peekNextKey();
      const auto settings = section_iter.getNext();

      for (auto iter = settings->cbegin(); iter != settings->cend(); iter++)
      {
         const auto type_name = iter->first;
         const auto arch_name = iter->second;

         Ogre::ResourceGroupManager::getSingleton()
               .addResourceLocation(arch_name,
                                    type_name,
                                    section_name);
      }
   }
